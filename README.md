...
- 좌컨셉/상단컨셉/2행컨셉 블록은 `삽입` 또는 블록 메뉴에서 추가합니다.
...
- AI 입력 중간의 머릿말 태그는 해당 위치 이후 페이지의 머릿말 변경 신호로 처리됩니다(페이지 나눔 위치에 따라 적용).
- 음영(정답) 블록은 자동으로 문서 끝으로 이동하며, 음영 블록끼리는 입력 순서를 유지합니다.
- 음영(정답) 블록은 AI 입력의 단별 문제 수 계산에 포함되지 않습니다.
- 꼬릿말 메타 태그는 저장만 되며 시험지 템플릿에서는 표시되지 않습니다(텍스트는 프리박스/표 템플릿 사용).
- 2행컨셉 라벨은 `[[2행컨셉_라벨]]`, 서브라벨은 `[[2행컨셉_라벨_서브라벨]]` 형식으로 입력합니다.

...
Concept blank behavior spec: `docs/concept-blank.md`.
...
- **개념 빈칸**: `[개념빈칸:#]정답[/개념빈칸]` (자동 번호, 빠른 정답 모음에 자동 수집).  
  ...
  - 개념 빈칸 정답 블록은 기본 1개로 자동 생성되며, 이동 가능하고 Alt+Shift+Enter로 분할됩니다(뒷 블록이 비면 자동으로 합쳐짐).
...
8) **프롬프트 다운로드**
- 사이드바 `프롬프트` 섹션에서 1차 데이터추출/2차 변환/콘텐츠 정의 프롬프트를 다운로드할 수 있습니다.
...
역할: 시험지 데이터(docData)와 편집 설정(settings), 실행 취소(History) 등 모든 상태 값을 관리합니다.

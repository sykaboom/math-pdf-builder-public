...
/**
 * Build a ZIP archive using store (no compression).
 * @param {Array<{path: string, data: Uint8Array, modifiedAt?: Date}>} entries
 * @returns {Uint8Array}
 */
...
const readZip = (input) => {
    ...
    if (eocdOffset < 0) throw new Error('ZIP 끝 레코드를 찾을 수 없습니다.');
    ...
    for (let i = 0; i < totalEntries; i++) {
        ...
        if (signature !== 0x02014b50) throw new Error('ZIP 중앙 디렉터리 오류');
        ...
        if (localSig !== 0x04034b50) throw new Error('ZIP 로컬 헤더 오류');
        const localNameLen = view.getUint16(localOffset + 26, true);
        const localExtraLen = view.getUint16(localOffset + 28, true);
        const dataOffset = localOffset + 30 + localNameLen + localExtraLen;
        const fileData = data.slice(dataOffset, dataOffset + compressedSize);

        if (compression !== 0) throw new Error(`압축 방식 미지원: ${compression}`);
